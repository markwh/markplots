...
library(knitr)
opts_chunk$set(eval = FALSE, cache = TRUE)
library(ggthemr)
...
ggplot(diamonds, aes(x=carat, y=price, color = clarity)) + 
  geom_point(shape = 23, size = 1.5, alpha = 0.75) +
  mytheme3$theme +
  # mytheme$scales$scale_colour_discrete()
  mytheme3$scales$scale_colour_discrete()
...
Cool! Time to save this one somewhere I can access it. It may go into a package someday, but for now I'll just save the .Rda. 
...
Next I want to play around with a different package, [colourlovers](https://github.com/leeper/colourlovers), which provides access to the [COLOURlovers](http://www.colourlovers.com/) API. 
...
Next I'm going to package these up. I'm going to do this in a package I keep for plotting, [markplots](http://github.com/markwh/markplots). I'll have to add my theme:
...
Now I'm going to try a different approach using functions in the **colorspace** package. I also want to try a new palette I created on [colourlovers](http://www.colourlovers.com/). 
...
getheme_dark <- ggthemr(palette = gepalette_dark, type = "outer", 
                        line_weight = 0.75,
                        set_theme = FALSE)

# Edit panel grid
getheme_dark$theme <- getheme_dark$theme + 
  theme(panel.grid.major = element_line(linetype = 4, size = 0.5))
```
...
diamonds %>% 
  ...
  geom_point(shape = 21, size = 2, alpha = 0.7, stroke = 0.1) +
  ermatheme_dark +
  getheme_dark$scales$scale_fill_discrete() +
  ggtitle("This is the plot title. Lorem ipsum et cetera")
```
